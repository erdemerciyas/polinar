...
*   **Görsel Dil Katmanları:** Temiz beyaz ve gri alanlar, endüstriyel koyu lacivert zeminler, dikkat çekici ve dinamik altın sarısı vurgular.

---

## 2. Logo Kullanımı

*   **Ana Logo:** Üzerinde (R) tescil işareti bulunan, O harfi içinde şemsiye/sekizgen form barındıran Polinar Altın Sarısı tipografik logo.
...
1.  **Polinar Altın Sarısı (Primary Action Gold)**
    *   **Kullanım:** Logoda, ana aksiyon butonlarında ("Subscribe", "Contact"), aktif menü öğelerinde, scroll edilmiş navbar arka planında ve başlık altı ayırıcı çizgilerde vurgu rengi olarak.
    *   **HEX:** `#EDBA13` (Primary), `#D4A80E` (Dark/Hover), `#B8920A` (Deep/Pressed)
    *   **Kontrast Notu:** Parlak altın zemin üzerinde beyaz metin WCAG AA'yı karşılamaz. Altın zemin üzerinde koyu lacivert (`#0A1128`) veya siyah (`#222`) metin kullanılmalıdır.
...
2.  **Machinery (Customized Products) — Polinar Altın Sarısı**
    *   **Referans:** Broşürlerde Polinar'ın ana kurumsal altın sarısı rengi kullanılır.
    *   **HEX (Primary):** `#EDBA13` (polinar-mustard) — ana kurumsal renkle aynı.
    *   **HEX (Dark/Hover):** `#D4A80E` (polinar-mustard-dark)
...
**Tailwind Token'ları:** `polinar-mustard`, `polinar-mustard-dark`, `polinar-gold-deep`, `moulds-gold`, `moulds-gold-dark`, `pte-cyan`, `pte-cyan-dark`
**CSS Class'ları:** `.div-mustard`, `.div-gold`, `.div-cyan`, `.btn-primary`, `.btn-primary-gold`, `.btn-primary-cyan`, `.mould-category-card-gold`, `.mould-category-card-cyan`
...
*   **İkonlar:** Flat (düz) ve çok renkli (material) değil, ince çizgisel (outline) veya tam kapalı (solid) ikonlar. İkon renkleri İletişim Mavisi veya Polinar Altın Sarısı olmalıdır (örn. Whatsapp butonu yeşil, iletişim mail/tel ikonları mavi veya beyaz gibi amaca göre ayrıştırılmış).

---

## 6. Web UI (Arayüz) Bileşenleri

*   **Header & Navigasyon:** Çok sade. Logoya solda ağırlık verilir, ortada siyah (#222) linkler, *Hover (üzerine gelince)* ve *Active* durumdaki sayfanın linki Polinar Altın Sarısı olur. Scroll edildiğinde navbar arka planı altın sarısı olur, metin ve logo koyu laciverte döner. En sağda dil seçenekleri (EN, TR, RU vs.) sade harflerle konumlandırılır.
*   **Butonlar:** 
    *   *Primary Button (Örn: Subscribe):* Arka plan altın sarısı, yazı koyu lacivert (`#0A1128`). Keskin köşeli veya çok hafif yuvarlatılmış (border-radius: 2px - 4px) olmalı, çok yuvarlak olmamalıdır.
    *   *Kart Seçimleri:* Kenarlığı hardal sarısı hatlı (border), içi beyaz olan seçim kartları.
*   **Ayırıcı Çizgiler (Dividers):** Başlıkların altında veya "About US" bölümünde olduğu gibi asimetrik iki bölümden oluşan çizgiler (sol yarısı hardal sarısı, sağ tarafı gri olan stilize icon/ayraçlar) marka imzası olarak sayfalarda tekrar kullanılmalıdır.
*   **Ürün Çerçeveleri:** Ürün fotoğraflarının çevresinde koyu hardal sarısı tonlarında ince (<1px) bir çerçeve (border) ve hover edildiğinde belirginleşen ince bir gölge (box-shadow) kullanılmalıdır.
